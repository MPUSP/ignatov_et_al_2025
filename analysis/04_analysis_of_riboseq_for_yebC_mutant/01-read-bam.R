...
read_bam <- function(sample_names = c("wt_1"), read_min = 27, read_max = 40){
  ...
  for(i in 1:length(sample_names)){
    ...
    pos_plus <- end(bam_plus) - 15
    pos_minus <- start(bam_minus) + 15
    ...
    wig_minus_header <- paste('track type=wiggle_0 name=', sample_name, '_minus color=0,0,255',
                              sep = "")
    wig_minus <- cvg_df |>
      select(nt_genome_coord, cvg_minus) |>
      mutate(cvg_minus = -cvg_minus) |> 
      filter(cvg_minus != 0)
...
write_csv(rsd, file = "riboseq.csv")
